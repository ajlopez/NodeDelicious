...
function getToken(cb) {
    var options = {
        hostname: 'avosapi.delicious.com',
        port: 443,
        path: '/api/v1/oauth/token?client_id=' + process.env.DELICIOUS_CLIENT + '&client_secret=' + process.env.DELICIOUS_SECRET + '&grant_type=credentials&username=' + process.env.DELICIOUS_USER + '&password=' + process.env.DELICIOUS_PASSWORD,
        method: 'POST'
    };
    ...
    var req = https.request(options, function(res) {
        ...
        res.on('end', function(d) {
            ...
            cb(null, JSON.parse(buffer).access_token);
        });
    });
...
function deletePost(link, cb) {
    var path = "/v1/posts/delete";

    var query = '';

    query = addToQueryString(query, 'url', link);
...
function addPost(link, desc, cb) {
    var path = "/v1/posts/add";

    var query = '';

    query = addToQueryString(query, 'url', link);
    query = addToQueryString(query, 'description', desc);
